...
const CartItems = ({ cart, handleRemoveItem, handleQuantityChange, discountAmount, totalCost }) => {
  return (
    <div className="lg:w-1/2 p-5">
      <h3 className="mb-8 pt-2 text-center text-2xl font-bold uppercase">Giỏ hàng</h3>
      {cart?.products?.map((item) =>
        item.productId ? (
          <div className="flex items-center mb-6 shadow-xl" key={item.productId._id}>
            ...
            <div className="ml-4 flex-grow">
              ...
              {item.productId.discount !== undefined && (
                <p className="text-sm text-gray-500">Giảm giá: {item.productId.discount}%</p>
              )}
      ...
      <div>
        <div className="flex justify-between p-3 text-white rounded-md bg-gradient-to-tl from-lime-200 via-sky-500 to-[#b9f372] m-2">
          ...
          <h5 className="font-semibold">{discountAmount.toLocaleString()} VND</h5>
        </div>
